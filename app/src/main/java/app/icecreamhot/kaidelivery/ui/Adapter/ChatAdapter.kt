...
class ChatAdapter(val arrChat: ArrayList<ChatMessage>, val userImg: String, val empImg: String): RecyclerView.Adapter<RecyclerView.ViewHolder>() {
    ...
    override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): RecyclerView.ViewHolder {
        return if(viewType == TYPE_CHAT_EMPLOYEE) {
            ViewHolderEmployee(LayoutInflater.from(parent.context).inflate(R.layout.chat_from_layout, parent, false))
        } else {
            ViewHolderUser(LayoutInflater.from(parent.context).inflate(R.layout.chat_to_layout, parent, false))
        }
    }
